...
def skeleton_synapse_visualization(body_Id, type_pre=None, type_post=None, rois_pre=None, rois_post=None, top=None, primary_only=True, skeleton_color=bokeh.palettes.Inferno3[0], pre_palett=None, post_palett=None, loop_colors=True, dim=2, synapse_size=None, batch_size=None):
    ...
    """
    # TODO: add support for all pre/post types (optionally color by type or instance)
    # TODO: add support for instances (color by bodyId)
...
# maybe split into 2 functions: fetch synapses and pliot skeleton
def skeleton_synapse_visualization_flex(body_Id, pre_scale=None, post_scale=None, pre_id=None, post_id=None, pre_lable_res=None, post_lable_res=None, rois_pre=None, rois_post=None, top=None, primary_only=True, skeleton_color=bokeh.palettes.Inferno3[0], pre_palett=None, post_palett=None, loop_colors=True, dim=2, synapse_size=None, batch_size=None):
    ...
    if pre_scale: assert pre_scale in ['instnace', 'post', 'all'], "Error: must specify presynaptic scale of 'instance', 'type', or 'all'"
    if post_scale: assert post_scale in ['instnace', 'post', 'all'], "Error: must specify postsynaptic scale of 'instance', 'type', or 'all'"
    neuron_cri = neuprint.NeuronCriteria(bodyId=body_Id)
    p = figure()
    p.y_range.flipped = True
    s = neuprint.skeleton.fetch_skeleton(body_Id, format='pandas')
    ...
    post_top_conns = None
    if pre_scale:
        if pre_scale=='instance':
            upstream_cri = neuprint.NeuronCriteria(instance=pre_id)
            if pre_lable_res==None: pre_lable_res = 'bodyId'
        elif pre_scale=='type':
            upstream_cri = neuprint.NeuronCriteria(type=pre_id)
            if pre_lable_res==None: pre_lable_res = 'instance'
        else:
            upstream_cri = None
            if pre_lable_res==None: pre_lable_res = 'type'
        if pre_lable_res=='neuron': pre_lable_res = 'bodyId'
        pre_syn_cri = neuprint.SynapseCriteria(rois=rois_pre, primary_only=primary_only)
        print("Fetching pre-synaptic connections...")
        pre_conn_df = neuprint.fetch_synapse_connections(upstream_cri, neuron_cri, pre_syn_cri)
        pre_neurons, _ = neuprint.fetch_neurons(pre_conn_df['bodyId_pre'].unique())
