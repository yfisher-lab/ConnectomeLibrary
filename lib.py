...
def gen_syn_scatter(body_Id, scale, conn_id, conn_type, rois=None, lable_res=None, top=None, primary_only=True, palett=None, loop_colors=True):
    ...
    if scale=='instance':
        ...
        if lable_res==None: lable_res = 'instance'
    else:
        conn_cri = None
        if lable_res==None: lable_res = 'type'
    if lable_res=='neuron': lable_res = 'bodyId'
    syn_cri = neuprint.SynapseCriteria(rois=rois, primary_only=primary_only)
    if conn_type == 'pre':
        pre_cri = conn_cri
        post_cri = neuron_cri
    else:
        pre_cri = neuron_cri
        post_cri = conn_cri
    print(f"Fetching {conn_type}-synaptic connections...")
    conn_df = neuprint.fetch_synapse_connections(pre_cri, post_cri, syn_cri)
    neurons, _ = neuprint.fetch_neurons(conn_df['bodyId_'+conn_type].unique())
    conn_df = neuprint.utils.merge_neuron_properties(neurons, conn_df)
    if top: 
        top_conns = conn_df[lable_res+'_'+conn_type].value_counts().head(top)
    else:
        top_conns = conn_df[lable_res+'_'+conn_type].value_counts()
    if not palett:
        lc = len(top_conns)
        if lc <= 11:
            palett = bokeh.palettes.Plasma[lc if lc>2 else 3]
        elif lc > 100:
            palett = bokeh.palettes.Plasma256
        else:
            if loop_colors:
                palett = bokeh.palettes.Plasma11
            else:
                palett = bokeh.palettes.Iridescent23
    points = conn_df.query(f'{lable_res}_{conn_type} in @top_conns.index').copy()
    colors = (palett * (len(points) // len(palett) + 1))[:len(points)]
    points['color'] = points[lable_res+'_'+conn_type].map(dict(zip(top_conns.index, colors)))
    return top_conns, points


def skeleton_synapse_visualization_flex(body_Id, pre_scale=None, post_scale=None, pre_id=None, post_id=None, pre_lable_res=None, post_lable_res=None, rois_pre=None, rois_post=None, top=None, primary_only=True, skeleton_color=bokeh.palettes.Inferno3[0], pre_palett=None, post_palett=None, loop_colors=True, dim=2, synapse_size=None, batch_size=None):
    """ Function returning a graphic of the skeleton of neuron specified by body_Id with the desired synapses plotted colored by pre/post synpase and neuron subtype
        ...
        * pre_scale (str): scale at which to examine synapses onto the given neuron from
            - 'instance': anylize synapses onto the specified neuron from a given presynaptic instance (subtype)
                * NOTE: must specify neuprint neuron instance (subtype) name as 'pre_id' argument
            - 'type': anylize synapses onto the specified neuron from given presynaptic type
                 * NOTE: must specify neuprint neuron type name as 'pre_id' argument
            - 'all': anylize synapses onto the specified neuron from all presynaptic neurons
            - None: don't analyze pre synaptic connections
        * post_scale (str): scale at which to examine synapses from the given neuron onto
            - 'instance': anylize synapses from the specified neuron onto a given postsynaptic instance (subtype)
                * NOTE: must specify neuprint neuron instance (subtype) name as 'post_id' argument
            - 'type': anylize synapses from the specified neuron onto given postsynaptic type
                 * NOTE: must specify neuprint neuron type name as 'post_id' argument
            - 'all': anylize synapses from the specified neuron onto all postsynaptic neurons
            - None: don't analyze post synaptic connections
        * pre_id (str): neuprint identifier for presynaptic neuron instance/type
        * post_id (str): neuprint identifier for postsynaptic neuron instance/type
        * pre_lable_res (str): resolution at which to label presynapses
            - 'neuron': must have a minimum pre_scale of 'instance'
            - 'instance': must have a minimum pre_scale of 'type'
            - 'type': must have a minimum pre_scale of 'all'
        * post_lable_res (str): resolution at which to label postsynapses
            - 'neuron': must have a minimum post_scale of 'instance'
            - 'instance': must have a minimum post_scale of 'type'
            - 'type': must have a minimum post_scale of 'all'
    ...
    """
    # TODO: add support for lists of pre/post synaptic neuron instances/types 
    ...
    post_top_conns = None
    if pre_scale:
        pre_top_conns, pre_points = gen_syn_scatter(body_Id, scale=pre_scale, conn_id=pre_id, conn_type='pre', rois=rois_pre, lable_res=pre_lable_res, top=top, primary_only=primary_only, palett=pre_palett, loop_colors=loop_colors)
        p.scatter(pre_points['x_pre'], pre_points['z_pre'], color=pre_points['color'])
    if post_scale:
        post_top_conns, post_points = gen_syn_scatter(body_Id, scale=post_scale, conn_id=post_id, conn_type='post', rois=rois_post, lable_res=post_lable_res, top=top, primary_only=primary_only, palett=post_palett, loop_colors=loop_colors)
